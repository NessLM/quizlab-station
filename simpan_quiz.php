...
//     lokasi -> nama scene aktif = penanda tempat quiz
...
// Fungsi bantu: kirim balasan JSON lalu berhenti
...
// 3) Beri nilai default biar tidak kosong (Unity pun sudah kirim "-")
if ($nama   === '') $nama   = '-';
if ($kelas  === '') $kelas  = '-';
...
$stmt = mysqli_prepare($koneksi, 'INSERT INTO hasil_quiz (nama, kelas, score, lokasi) VALUES (?, ?, ?, ?)');
mysqli_stmt_bind_param($stmt, 'ssis', $nama, $kelas, $score, $lokasi);
...
// 6) Berhasil
balas('sukses', "Data tersimpan: nama=$nama, kelas=$kelas, score=$score, lokasi=$lokasi", $id);
